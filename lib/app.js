...
var mysql = require('mysql'),
    ...
    TagsMigrator = require('./TagsMigrator'),
    Q = require('q');
...
function migrateTags() {
  ...
  var migrator = new TagsMigrator(migratorOptions);
  ...
  // Get the tag details for each tag
  var query = connection.query(
    'SELECT ' +
    ' entity_tag.feed_post_id AS feed_post_id, entity_tag.tag_id AS tag_id, entity_tag.date_created AS date_created, ' +
    ' employee.id AS tagged_by_id, employee.nme AS tagged_by_name, ' +
    ' tag.nme AS tag_name ' +
    'FROM entity_tag ' +
    'JOIN employee ' +
    ' ON employee.id = entity_tag.tagged_by_employee_id ' +
    'JOIN tag ' +
    ' ON tag.id = entity_tag.tag_id ' +
    'WHERE entity_tag.feed_post_id IS NOT NULL '
  );
...
migrateFeedPosts().then(migrateLikes).then(migrateTags).finally(cleanUp).done();
